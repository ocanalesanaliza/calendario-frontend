...
describe('areaTemplatesService', () => {
  ...
  it('uses the exact template collection, detail, version, and archive contracts', async () => {
    ...
    await createAreaTemplate({ name: 'Apertura', description: '', tasks: [task] })
    await getAreaTemplate(3)
    await getAreaTemplateVersions(3)
    await createAreaTemplateVersion(3, { tasks: [task] })
    await archiveAreaTemplate(3)

    expect(apiRequest).toHaveBeenNthCalledWith(1, '/api/calendar/area-templates/')
    expect(apiRequest).toHaveBeenNthCalledWith(2, '/api/calendar/area-templates/', { method: 'POST', body: JSON.stringify({ name: 'Apertura', description: '', tasks: [task] }) })
    ...
    expect(apiRequest).toHaveBeenNthCalledWith(5, '/api/calendar/area-templates/3/versions/', { method: 'POST', body: JSON.stringify({ tasks: [task] }) })
    expect(apiRequest).toHaveBeenNthCalledWith(6, '/api/calendar/area-templates/3/archive/', { method: 'POST' })
  })
})
