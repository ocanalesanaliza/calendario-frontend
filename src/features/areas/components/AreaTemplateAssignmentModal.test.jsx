...
const service = vi.hoisted(() => ({ assignAreaTemplate: vi.fn(), cancelPending: vi.fn(), getAssignableAreaTemplates: vi.fn(), getState: vi.fn() }))
...
describe('AreaTemplateAssignmentModal', () => {
  beforeEach(() => { Object.values(service).forEach((mock) => mock.mockReset()); service.getAssignableAreaTemplates.mockResolvedValue([{ id: 9, name: 'Apertura', description: 'Sin historial', current_version: { number: 2 } }]); service.getState.mockResolvedValue({ actions: { replace_template: true, cancel_future_template_assignment: true }, effective_assignment: { template: { name: 'Actual', current_version: { number: 1 } } }, pending_assignment: { id: 12, template: { name: 'Futura' }, fecha_inicio: '2026-10-01' } }); service.assignAreaTemplate.mockResolvedValue({}); service.cancelPending.mockResolvedValue({}) })
  ...
  it('carga el estado, reemplaza con fecha opcional y cancela una asignación futura confirmada', async () => {
    ...
    await waitFor(() => expect(service.assignAreaTemplate).toHaveBeenCalledWith(3, { template_id: 9, version: 2, fecha_inicio: '2026-10-15' }))
  ...
  it('asigna una plantilla cuando no existe una asignación efectiva', async () => {
    service.getState.mockResolvedValue({ actions: ['assign_template'] })
    render(<AreaTemplateAssignmentModal area={area} onClose={vi.fn()} />)

    const templateSelect = await screen.findByLabelText('Plantilla')
    fireEvent.change(templateSelect, { target: { value: '9' } })
    fireEvent.click(screen.getByRole('button', { name: 'Asignar plantilla' }))

    await waitFor(() => expect(service.assignAreaTemplate).toHaveBeenCalledWith(3, { template_id: 9, version: 2 }))
  })
})
